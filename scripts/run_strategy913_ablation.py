...
ABLATION_MODES: dict[str, dict[str, bool]] = {
    "control": {"staged_entry": False, "early_protect": False, "reentry": False},
    "stage_only": {"staged_entry": True, "early_protect": False, "reentry": False},
    "protect_only": {"staged_entry": False, "early_protect": True, "reentry": False},
    "reentry_only": {"staged_entry": False, "early_protect": False, "reentry": True},
    "stage_protect": {"staged_entry": True, "early_protect": True, "reentry": False},
    "stage_reentry": {"staged_entry": True, "early_protect": False, "reentry": True},
    "protect_reentry": {"staged_entry": False, "early_protect": True, "reentry": True},
    "all_three": {"staged_entry": True, "early_protect": True, "reentry": True},
}
...
def install_beast_patch(h: Any, mode: str):
    features = ABLATION_MODES[mode]
    ...
    watch_code = (
        '        if ENABLE_REENTRY and reason in ("EARLY_FAILURE", "FOLLOWTHROUGH_15M") '
    ...
    source = _replace_once(
        ...
        '            required = target_margin * (STAGE1_FRACTION if ENABLE_STAGE else 1.0)\n',
        "stage1 margin",
    )
...
'''
    new_trail = '''            proposed = None
            if ENABLE_PROTECT and pos["mfe"] >= EARLY_PROTECT_TRIGGER_R * pos["risk"]:
                protect = pos["entry"] * (
                    1.0 - direction * EARLY_PROTECT_STOP_R * pos["risk"]
                )
                proposed = protect
            if ENABLE_PROTECT and pos["mfe"] >= BREAK_EVEN_TRIGGER_R * pos["risk"]:
                be = pos["entry"] * (1.0 + direction * BE_LOCK)
                proposed = (
                    max(proposed, be)
    ...
    namespace.update(
        {
            ...
            "ENABLE_STAGE": features["staged_entry"],
            "ENABLE_PROTECT": features["early_protect"],
            "ENABLE_REENTRY": features["reentry"],
        }
    )
...
def _prepare(
    ...
    if window == "may_2026":
        ...
        for run_fn in variant_runs.values():
    ...
    raw = _prepare_generic(
        h,
        market_ref,
        (baseline_run, *variant_runs.values()),
        config,
    )
    return raw, baseline_run
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="novabot913-ablation-") as tmp:
        directory = Path(tmp)
        ...
        market_ref = modules["tmp_hybrid_archive_market"]
        variant_runs = {
            mode: install_beast_patch(h, mode)
            for mode in ABLATION_MODES
        }
        raw, baseline_run = _prepare(args.window, modules, variant_runs)

        baseline_filter = _causal_filter_factory(h, market_ref)
        baseline = _run_engine(h, baseline_run, raw, baseline_filter)
        baseline_summary = _summary(baseline)
        print(
            "ABLATION_BASELINE=" + json.dumps(baseline_summary, sort_keys=True),
            flush=True,
        )
        ...
        for mode, run_fn in variant_runs.items():
            ...
            print(
                f"ABLATION_{mode.upper()}="
                + json.dumps(variant_summary, sort_keys=True),
                flush=True,
            )
            variants[mode] = {
                "features": ABLATION_MODES[mode],
                ...
                "trades": [
                    _trade_view(item)
                    for item in variant["trades_detail"]
                ],
            }

        control_parity = variants["control"]["summary"] == baseline_summary
        ...
        report = {
            "phase": "strategy_913_beast_ablation",
            "research_only": True,
            "adopted": False,
            ...
            "baseline_trades": [
                _trade_view(item)
                for item in baseline["trades_detail"]
            ],
            "variants": variants,
        }
        output = Path(f"strategy913_ablation_{args.window}.json")
        output.write_text(
            json.dumps(report, indent=2, sort_keys=True),
            encoding="utf-8",
        )
        print(
            "STRATEGY913_ABLATION=" + json.dumps(report, sort_keys=True),
            flush=True,
        )
